...
"""
Human-in-the-Loop (HITL) Workflow Comparison Experiment (Optimized with Peak-Local-Max)
Section 4.5 of the preprint paper

This script evaluates different counting workflows using peak-local-max optimized prediction:
...
This version uses peak-local-max counting (like test-opt.py) instead of raw density map sum.

By: Piyachet Pongsantichai
Created: 2025-01-17
Modified: 2025-01-17 (Added peak-local-max optimization)
"""
...
from utils import MAPS, Scales, Transform, extract_features, count_dots_with_adjusted_area
...
class ExperimentConfig:
    ...
    random_seed: int = 42
    confidence_alpha: float = 0.5  # Weight for variance component
    confidence_beta: float = 0.5   # Weight for correlation component
...
def compute_local_variance(heatmap: np.ndarray, peak_coords: np.ndarray, 
    ...
    # Map to 0-1 range (empirically determined)
    return min(1.0, avg_var * 10)


def compute_correlation_score(heatmap: np.ndarray, 
                              thresholds: Tuple[float, float, float] = (0.7, 0.65, 0.3),
                              min_distance: int = 10) -> float:
    """
    ...
    # Penalize large differences between thresholds (instability)
    stability = 1.0 - min(1.0, abs(n_low - n_medium) / max(n_low, 1))
    
    return 0.6 * consistency + 0.4 * stability
...
def compute_confidence_score(heatmap: torch.Tensor, 
    ...
    C = alpha * C_variance + beta * C_correlation
    
    where C_variance is normalized inverse variance (higher for lower variance)
    and C_correlation is the normalized correlation score.
    ...
    confidence = alpha * c_variance + beta * correlation_score
    ...
    # Unique peaks
    all_coords = np.vstack([high_peaks, medium_peaks, low_peaks]) if len(high_peaks) > 0 and len(medium_peaks) > 0 and len(low_peaks) > 0 else low_peaks
    if len(all_coords) > 0:
...
def run_model_inference(config: ExperimentConfig, 
    ...
    Uses peak-local-max optimization for counting (matching test-opt.py).
    """
    # Load models
    ...
    for im_id in pbar:
        ...
        # Get prediction using peak-local-max optimization (matching test-opt.py)
        density_sum = output.sum().item()  # Keep raw density sum for reference
        _, _, _, pred_count = count_dots_with_adjusted_area(
        ...
        pbar.set_description(f"GT: {gt_count}, Pred: {pred_count}, Conf: {conf_info['confidence_score']:.3f}")
...
def save_results(results: List[ImageResult], 
    ...
    summary_lines = [
        "=" * 80,
        "HITL Workflow Comparison Experiment Results (Optimized with Peak-Local-Max)",
...
def main():
    parser = argparse.ArgumentParser(description="HITL Workflow Comparison Experiment (Optimized)")
    ...
    # 3. Adaptive HITL at different thresholds
    for threshold in [0.6, 0.7, 0.8, 0.9]:
        print(f"\n[{len(workflow_results)+1}/9] Evaluating Adaptive HITL (θ={threshold})...")
        workflow_results.append(evaluate_adaptive_hitl(results, config, threshold))
    
    # 4. Full Manual Verification
    print(f"\n[{len(workflow_results)+1}/9] Evaluating Full Manual Verification...")
