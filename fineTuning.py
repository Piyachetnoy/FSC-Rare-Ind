...
from model import  FamNet,CountRegressor,weights_normal_init
...
def train():
    ...
    for im_id in pbar:
        ...
        with torch.no_grad():
            features = extract_features(famnet, image.unsqueeze(0), boxes.unsqueeze(0), MAPS, Scales)
...
def eval():
    ...
    for im_id in pbar:
        ...
        with torch.no_grad():
            output = regressor(extract_features(famnet, image.unsqueeze(0), boxes.unsqueeze(0), MAPS, Scales))
...
for epoch in range(0,args.epochs):
    ...
    stats_file = join(args.output_dir, "stats-RareIndt" +  ".txt")
    ...
    if best_mae >= val_mae:
        best_mae = val_mae
        best_rmse = val_rmse
        model_name = args.output_dir + '/' + "FamNet-RareIndt.pth"
