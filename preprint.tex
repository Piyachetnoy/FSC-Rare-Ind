...
Full Manual Verification & 2.115 & 5.832 & 100 & 36.3 \\ \hline
\end{tabular}%
}
\caption{\textbf{Workflow Comparison Results (INDT-576 Test Set, INDT-409-trained model).}}
\label{tab:workflow_comparison}
\end{table}

The results demonstrate that the adaptive HITL approach achieves superior efficiency compared to random sampling at equivalent accuracy levels. For example, at approximately 30\% HIR, adaptive HITL achieves MAE of 6.234, whereas random sampling requires 50\% HIR to reach similar accuracy (MAE 5.418). \par
...
\begin{itemize}
\item \textbf{For high-accuracy requirements (MAE < 5):} Use adaptive HITL with $\theta=0.7$, accepting $\sim$58\% human intervention rate.
\item \textbf{For balanced operations (MAE < 7):} Use adaptive HITL with $\theta=0.5$, requiring $\sim$32\% human intervention rate.
\item \textbf{For high-efficiency operations (MAE < 9):} Use adaptive HITL with $\theta=0.3$, requiring only $\sim$15\% human intervention rate.
\end{itemize}
...
Experimental results demonstrate that fine-tuned FamNet models achieve substantial accuracy improvements on industrial objects (MAE < 10), though with some reduction in general object counting ability. Our adaptive HITL workflow achieves equivalent accuracy to random sampling with significantly lower human intervention rates (e.g., 32\% vs. 50\% HIR for MAE $\sim$6). \par
