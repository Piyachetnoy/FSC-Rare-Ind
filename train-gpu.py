...
parser.add_argument("-ts", "--test-split", type=str, default='val', choices=["train", "test", "val"], help="what data split to evaluate on")
...
def train():
    ...
    for im_id in pbar:
        ...
        image = Image.open(f'{im_dir}/{im_id}')
        image.load()
        density_path = f'{gt_dir}/{im_id.split(".jpg")[0]}.npy'
        ...
        pbar.set_description(f'actual-predicted: {gt_cnt:6.1f}, {pred_cnt:6.1f}, error: {cnt_err:6.1f}. Current MAE: {train_mae/cnt:5.2f}, RMSE: {(train_rmse/cnt)**0.5:5.2f} Best VAL MAE: {best_mae:5.2f}, RMSE: {best_rmse:5.2f}')
...
def eval():
    ...
    for im_id in pbar:
        ...
        image, boxes = sample['image'].to('mps'), sample['boxes'].to('mps')
